...
def task_plot_normal_potential(
    ...
    for i, (gamma_u, gamma_l, delta) in enumerate(
        ...
        ax.plot(
            ...
            c=f"C{i}",
            label=rf"$\InterfaceEnergy_{{\Upper}}=\qty{{{gamma_u}}}{{\joule\per\square\meter}}, \InterfaceEnergy_{{\Lower}}=\qty{{{gamma_l}}}{{\joule\per\square\meter}}, \SurfaceVectorAngle_{{\Normal}}=\qty{{{delta}}}{{\degree}}$",
        )
        ax.plot(
            ds,
    ...
    ax.set_xlabel(r"$\Step\Shift_{\Normal}$ in \unit{\meter}")
    ax.set_ylabel(r"$\Step\GibbsEnergy_{\Normal}$ in \unit{\joule\per\meter}")
    ax.grid(True)
