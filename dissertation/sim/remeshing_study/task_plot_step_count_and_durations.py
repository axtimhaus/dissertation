...
def task_plot_step_count_and_durations(
    ...
    ax.bar(labels, step_counts, label="Step Count", color="C0", width=BAR_WIDTH, align="edge")

    times = [float(f.read_text()) for key, f in time_files.items()]
    # ax2.bar(labels, times, label="Simulation Duration", color="C1", width=-BAR_WIDTH, align="edge")
