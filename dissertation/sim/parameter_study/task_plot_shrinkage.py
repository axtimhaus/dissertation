...
for study in STUDIES:
        ...
        def task_plot_shrinkage(
            ...
            for key, df in data_frames.items():
                particle1: pd.DataFrame = df.filter(pc.field("Particle.Id") == PARTICLE1_ID.bytes).group_by(["State.Id"]).aggregate([("State.Time", "one"), ("Particle.Coordinates.X", "one"), ("Particle.Coordinates.Y", "one")]).sort_by("State.Time_one").to_pandas()
