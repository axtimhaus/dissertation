...
\Cref{fig:model_development/surface_normal} shows the principal geometry of a surface node and its normal mode of shifting.
...
The change of Gibbs energy due to normal node shifting is described by \cref{eq:gibbs-diff-surface-normal} with the prime values as measures after shifting.
\begin{equation}
...
The normal vector points under an angle of $\SurfaceVectorAngle_\Normal$ acc.~to~\cref{eq:delta-surface-normal} to both surface lines.
With a certain normal shift ${\Step\Shift}_\Normal$, the surface lengths after shifting are then given by \cref{eq:surface-distance-shifted-normal}.
\begin{equation}
...
\begin{subequations}
    \label[pleq]{eq:surface-distance-shifted-normal}
    \label{eq:surface-distance-shifted-normal-sg}
    \begin{align}
        \SurfaceDistance_{\Upper}' & = \sqrt{\SurfaceDistance_{\Upper}^2 + \Step\Shift_{\Normal}^2 - 2 \SurfaceDistance_{\Upper} \Step\Shift_{\Normal} \cos \SurfaceVectorAngle_{\Normal}}
        \label{eq:surface-distance-shifted-normal-upper} \\
        \SurfaceDistance_{\Lower}' & = \sqrt{{\SurfaceDistance}_{\Lower}^2 + {\Step\Shift}_{\Normal}^2 - 2 {\SurfaceDistance}_{\Lower} {\Step\Shift}_{\Normal} \cos \SurfaceVectorAngle_{\Normal}}
        \label{eq:surface-distance-shifted-normal-lower}
    \end{align}
\end{subequations}

\Cref{fig:model_development/plot_normal_potential} shows the change in Gibbs energy due to normal shifting with use of \crefrange{eq:gibbs-diff-surface-normal}{eq:surface-distance-shifted-normal-sg} with different values of $\SurfaceVectorAngle_{\Normal}$ and $\InterfaceEnergy$, alongside the linear approximation around ${\Step\Shift}_{\Normal} = 0$.
...
The partial derivative of Gibbs energy is obtained as the limit of $\Step\GibbsEnergy_{\Normal}$ when ${\Step\Shift}_{\Normal} \rightarrow 0$ as in \cref{eq:gibbs-partial-surface-normal}.
\begin{equation}
...
The partial derivative of volume is obtained by the limit of $\Step\Volume_{\Normal\Upper} + \Step\Volume_{\Normal\Lower}$ when ${\Step\Shift}_{\Normal} \rightarrow 0$ as in \cref{eq:volume-partial-surface-normal}, where $\Step\Volume_{\Normal\Upper}$ and $\Step\Volume_{\Normal\Lower}$ are the areas of the triangles formed by $\SurfaceDistance$, $\SurfaceDistance'$ and $\Step\Shift_{\Normal}$ on the upper and lower side as given by \cref{eq:volume-diff-surface-normal}.
\begin{subequations}
    \label[pleq]{eq:volume-diff-surface-normal}
    \begin{align}
...
Regarding the tangential shifting, the direction vector is under an angle of $\SurfaceVectorAngle_{\Tangential}$ to the upper surface line, given by \cref{eq:delta-surface-tangential}.
The change in Gibbs energy is calculated similarly as in the normal case according to \cref{eq:gibbs-diff-surface-tangential}, but the shifted surface lengths are calculated as in the \cref{eq:surface-distance-shifted-tangential} in dependence on the tangential shift ${\Step\Shift}_{\Tangential}$.
...
\end{equation}\vspace{-2ex}
\begin{subequations}
    \label[pleq]{eq:surface-distance-shifted-tangential}
    \begin{align}
...
\Cref{fig:model_development/plot_tangential_potential} shows the change in Gibbs energy due to tangential shifting with different values of $\InterfaceEnergy$.
...
Again, the partial derivative of Gibbs energy is found by the limit when ${\Step\Shift}_{\Tangential} \rightarrow 0$ as in \cref{eq:gibbs-partial-surface-tangential}.
The partial derivative of volume in \cref{eq:volume-partial-surface-tangential} is found similarly as in the normal case, but the contribution of the lower side must here be counted negatively.
\begin{equation}
...
The case, where the grain boundary is located below the currently regarded node, is shown in \cref{fig:surface-node-shifting-neck}.
Here, one must distinguish between the shift vector angles $\SurfaceVectorAngle$ to the upper and the lower surface line.
For the shown case the angles are as in \cref{eq:shift-neck-angles}, if the grain boundary is, however, located above the node, than the values of the upper and lower angles have to be switched.
The respective partial derivatives are slightly modified with the distinct angles as in \crefrange{eq:gibbs-partial-neck-normal}{eq:volume-partial-neck-tangential}.
\begin{align}
