...
To circumvent this problem, several approaches have been taken, from which the \gls{CIT} (sometimes referred to as \gls{TIP}) and the thermodynamics of \gls{IVS} shall be discussed here in particular.
...
The central assumption of the classic treatment of non-equilibrium irreversible processes is the assumption of a local equilibrium, which is that in a certain small volume of the system, the equilibrium state is present despite the gradient to the next neighboring volume.
...
The volume cell must be small enough to fulfill this time scale condition, but large enough to be described sufficiently well as a continuum.
So in fact the time needed for restoration of a local equilibrium state can be neglected in comparison to the macroscopical evolution of the system.
...
Temperature can be assumed constant, as the energy dissipated due to dissipative processes is small compared to the thermal energy stored in the system (sintering body and surrounding furnace), so the system does not heat up significantly due to the energy dissipated.
...
This corresponds to the empirical description of diffusion by Fick's law (\autoref{eq:fick-first}).
\begin{equation}
...
As the microscopic nature of thermodynamical systems is often to complex to be described alone by the macroscopical observable and controllable \gls{EVS} $\Vect\ExternalStateVariable$ (such as temperature, pressure, internal energy and entropy), approaches only considering those neglect large amounts of internal processes and specifics of the state.
An enormously successfully applied approach is that of the introduction of \gls{IVS} $\Vect\InternalStateVariable$ which represent dissipative quantities in the system and can be freely chosen dependent on the desired precision and particularity of the model.
...
The formalism axiomatically associates an local accompanying equilibrium state in dependence on the local non-equilibrium \gls{EVS} and the \gls{IVS} to a volume cell at a respective point in time.
...
On the other hand, if $\DeborahNumber >> 1$, the variable can be assumed constant in regard on the equilibrium condition.
...
They show some kind of inertia within the respective time scale, which is equivalent to $\DeborahNumber \approx 1$.
Internal degrees of freedom must be accompanied with their own kinetic law in terms of a differential equation to be included in the simulation.
They may, but do not have to, include a dissipative contribution as \gls{IVS} do.

A numerical approach to obtain a system of ordinary differential equations from this formalism will be discussed in \autoref{sec:extremal-principle}.
