...
To circumvent this problem, several approaches have been developed, from which the \gls{CIT} (sometimes referred to as \gls{TIP}) and the thermodynamics of \gls[postunset]{IVS} shall be discussed here in particular.
...
This means that the characteristic time of local equilibration is much smaller than the characteristic time of the macroscopic process, expressed by the Deborah number $\DeborahNumber = \TimeNorm_{\text{local}} / \TimeNorm_{\text{global}} \ll 1$.
...
On the other hand, processes with a Deborah number $\DeborahNumber \gg 1$ can be regarded as quasi-static.
...
The gradient of the Gibbs' energy equals zero as denoted by \cref{eq:cit-gibbs-equilibrium}, when the system has reached equilibrium.
\begin{equation}
...
The dissipation $\Dissipation$ during evolution of the system is proportional to the product of the fluxes and their respective thermodynamic forces as given in \cref{eq:cit-dissipation}.
\begin{equation}
    \Dissipation \sim \frac{\partial \GibbsEnergy}{\partial \Vect\ExternalStateVariable} \cdot \Vect\Flux \ge 0
    \label{eq:cit-dissipation}
\end{equation}

A usual (and for many processes valid) assumption is linearity between the fluxes $\Vect\Flux$ and the thermodynamic driving forces $\frac{\partial \GibbsEnergy}{\partial \Vect\ExternalStateVariable}$ with a coefficient $\Vect{C}$ as in \cref{eq:cit-flux-force-relationship}.
...
The local change in Gibbs' energy is then given by \cref{eq:ivs-local-las-gibbs-derivative}.
\begin{equation}
...
Similar to the requirements of local equilibrium in \gls{CIT}, an \gls{IVS} can be considered to always have their local equilibrium value if $\DeborahNumber \ll 1$ holds.
On the other hand, if $\DeborahNumber \gg 1$, the variable can be assumed constant in regard to the equilibrium condition.
