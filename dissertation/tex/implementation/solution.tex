...
This method, however, is prone to numerical instabilities, which are often observed as alternating zick-zack like surface structures (see \cref{ch:numerical-behavior}).
...
This method requires the evaluation of the systems gradient resp.~it's Jacobian matrix, which is sparse and follows the general structure shown in \cref{fig:implementation/sparse_structure}.
...
The calculation of a Newton step requires the solution of the linear system in \cref{eq:newton-step}, where $J_{\LagrangeFunction}$ is the Jacobian of the Lagrangian and $\Grad \LagrangeFunction$ it's gradient.
...
Then the fluxes are obtained using Fick's first law (\cref{eq:fick-first}).
...
Then, the tangential displacement of the neck node calculates as in \cref{eq:estimate-neck-tangetial-displacement} using the flux balance there, with $\Flux_{\Surface}$ as the estimated flux to the adjacent surface node calculated as above and $i$ as the iteration index.
\begin{equation}
