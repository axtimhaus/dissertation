...
The angle difference is calculated in accordance to the equations in \cref{sec:surface-evolution} with $\Step\Shift = \dot\Shift \Step\Time$.
\begin{equation}
...
Then, the tangential displacement of the neck node is calculated as in \cref{eq:estimate-neck-tangetial-displacement} using the flux balance, with $\Flux_{\Surface}$ as the estimated flux to the adjacent surface node calculated as above and $i$ as the iteration index.
\begin{equation}
...
Using the form of the dissipation function in \cref{eq:dissipation-function-node} the new flux is obtained as in \cref{eq:estimated-neck-new-flux}.
...
This flux is then inserted in the iteration again in \cref{eq:estimate-normal-displacement} for the grain boundary nodes and \cref{eq:estimate-neck-tangetial-displacement} for the neck node till the fixed point is reached.
