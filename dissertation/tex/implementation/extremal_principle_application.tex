...
For each node involved in a contact, the conditions according to \autoref{eq:maintain-compact-particles-fixed} are applied.

To fix the position of a particle contact group in space, one of its particles must be constrained in its position by a constrained like \autoref{eq:fixed-particle-constraints}.
...
Further constraints of the like are possible, for example to enforce a certain movement or apply an external load on a particle to simulate assisted compaction.
