...
The indizes $\square_{\Upper}$ and $\square_{\Lower}$ are used to denote the upper resp.~lower neighbor of a node.
${\Nodes}$ is the set of all nodes present in the system and ${\Particle}s$ the set of all particles, respectively.
...
A more appropriate way is to define the node shifts $\Step\Shift_{\Normal}$ and $\Step\Shift_{\Tangential}$ as internal state velocities, so that the required constraints $\Vect\RequiredConstraint$ become linear (see \autoref{subsec:required-constraints}).
...
\begin{equation}
    \Dissipation = -\sum^{\Nodes} \left[ \frac{\partial \GibbsEnergy}{\partial \Shift_{\Normal}} \Step\Shift_{\Normal} + \frac{\partial \GibbsEnergy}{\partial \Shift_{\Tangential}} \Step\Shift_{\Tangential} \right]
...
\begin{equation}
    \DissipationFunction = \sum^{\Nodes} \frac{1}{2} \frac{\GasConstant\Temperature}{\MolarVolume{\VacancyConcentration}^\Standard} \left( \frac{\SurfaceDistance_{\Upper} {\Flux}_{\Upper}^2}{{\DiffusionCoefficient}_{\Upper}} + \frac{{\SurfaceDistance}_{\Lower} {\Flux}_{\Lower}^2}{{\DiffusionCoefficient}_{\Lower}}\right) \Step\Time
...
\begin{equation}
    \frac{\partial\Volume}{\partial{\Shift}_{\Normal}} {\Step\Shift}_{\Normal} + \frac{\partial\Volume}{\partial{\Shift}_{\Tangential}} {\Step\Shift}_{\Tangential} = \Step\Volume = \left( {\Flux}_{\Upper} + {\Flux}_{\Lower} \right) \Step\Time
...
\begin{subequations}
    \begin{align}
        \Step{\Particle}\Radius &= \frac{\partial {\Particle}\Radius}{\partial \Shift_{\Normal}} \Step\Shift_{\Normal} + \frac{\partial {\Particle}\Radius}{\partial \Shift_{\Tangential}} \Step\Shift_{\Tangential} \\
        \Step{\Particle}\Angle &= \frac{\partial {\Particle}\Angle}{\partial \Shift_{\Normal}} \Step\Shift_{\Normal} + \frac{\partial {\Particle}\Angle}{\partial \Shift_{\Tangential}} \Step\Shift_{\Tangential} \\
        \Step{\Particle}\RotationAngle &= \frac{\partial {\Particle}\RotationAngle}{\partial \Shift_{\Normal}} \Step\Shift_{\Normal} + \frac{\partial {\Particle}\RotationAngle}{\partial \Shift_{\Tangential}} \Step\Shift_{\Tangential}
    \end{align}
\end{subequations}
