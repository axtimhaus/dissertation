...
Below, the equations to calculate these in the current model will be derived.
...
Most simple approaches directly include the particle distance $\Distance$ as a state variable.
For the current case it is calculated as the Euclidean distance between the centers of two particles (\cref{eq:euclidean-particle-distance}).
With $\Distance_0$ as the distance in the initial state, the shrinkage is then directly obtained via \cref{eq:shrinkage-two-particle}.
\begin{equation}
    \Distance = \sqrt{\left( \X_2 - \X_1 \right)^2 + \left( \Y_2 - \Y_1 \right)^2}
    \label{eq:euclidean-particle-distance}
\end{equation}
\begin{equation}
...
Then, with $\Area_0$ as the polygon area in the initial state, the shrinkage is calculated by \cref{eq:shrinkage-multiple-particle}.
