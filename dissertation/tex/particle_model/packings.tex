...
For the current case it is calculated as the Euclidean distance between the centers of two particles:
\begin{equation}
    \Distance = \sqrt{\left( \X_2 - \X_1 \right)^2 + \left( \Y_2 - \Y_1 \right)^2}
    \label{eq:euclidean-particle-distance}
\end{equation}
With $\Distance_0$ as the distance in the initial state, the shrinkage is then directly obtained via
\begin{equation}
...
Then, with $\Area_0$ as the polygon area in the initial state, the shrinkage is calculated by
\begin{equation}
