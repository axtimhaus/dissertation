...
The location of each node in space is defined by a tuple of polar coordinates $(\Angle, \Radius)$, where $\Angle$ is the angle coordinate and $\Radius$ the radius (distance from origin).
...
The center of the particle is used as an anchor for defining a particle's position in space.
...
See \cref{fig:model_development/particle_coordinates} for illustration on this.
...
\Cref{fig:model_development/two_particle_contact} shows such a contact of two particles.
Until here, three types of nodes can be identified:
\begin{description}
    \item[Surface Nodes] forming the surface of a particle in contact to atmosphere or vacuum (black).
...
Since every particle has its own polar coordinate system, the node coordinates involved can be regarded in either particle's coordinate system.
The coordinate system used shall be denoted hereinafter by $\ldots^{|\ldots}$ superscripts where necessary as demonstrated in \cref{fig:model_development/two_particle_contact}, where the coordinates of one neck node are shown as $(\Angle^{\Regarding 1}, \Radius^{\Regarding 1})$ and $(\Angle^{\Regarding 2}, \Radius^{\Regarding 2})$.
