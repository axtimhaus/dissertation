...
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{img/model_development/element_geometry}
...
Its geometry and important quantities are displayed in \cref{fig:model_development/element_geometry}.
Their relations used in following calculations will be derived in this section.

To avoid cluttering the equations with numeric indices, the quantities belonging to the currently regarded node will be written without any index.
The indices $\ldots_{\Upper}$ and $\ldots_{\Lower}$ are used to denote the upper and lower neighbor of this node.

The surface distance between the adjacent nodes is calculated using the cosine law as in the \cref{eq:surface-distance}, with the angle distances $\Diff\Angle_{\Upper} = \Angle_{\Upper} - \Angle$ and $\Diff\Angle_{\Lower} = \Angle - \Angle_{\Lower}$.
The distances are always positive.
\begin{subequations}
    \label[pleq]{eq:surface-distance}
    \begin{align}
...
The angles between the surface lines and the radius vector (surface-radius-angles $\SurfaceRadiusAngle_{\Upper}$ and $\SurfaceRadiusAngle_{\Lower}$) are calculated given the node coordinates using the cosine law as in the \cref{eq:surface-radius-angles}.
...
This negative angle is required for the volume partial differential to change its sign accordingly as derived in \cref{sec:surface-evolution}.
\begin{subequations}
    \label[pleq]{eq:surface-radius-angles}
    \begin{align}
...
The new coordinates after shifting along a vector $\Step\Shift$ which is directed under an angle of $\SurfaceVectorAngle$ with the upper surface line are calculated using cosine and sine law as in the \cref{eq:new-node-coordinates}.
...
The shift vector is for now arbitrary, but will be concretized in \cref{sec:surface-evolution}.
\begin{subequations}
    \label[pleq]{eq:new-node-coordinates}
    \begin{align}
...
For sintering behavior analysis and evaluation of numerical error the volume of a particle has to be determined.
In the present 2D case the volume reduces to the area enclosed by the surface line, which is calculated by summing over all element triangles as in \cref{eq:particle-volume}, where $\Nodes$ is the set of all nodes the particle consists of.
Note that if undercuts occur this equation is still correct since the respective elements will be counted negative, due to $\sin \Diff\Angle_{\Upper} < 0$.
\begin{equation}
