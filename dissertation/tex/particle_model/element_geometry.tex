...
The surface distance between the adjacent nodes is calculated using the cosine law as in \cref{eq:surface-distance}, with the angle distances $\Diff\Angle_{\Upper} = \Angle_{\Upper} - \Angle$ and $\Diff\Angle_{\Lower} = \Angle - \Angle_{\Lower}$.
...
The angles between the surface lines and the radius vector (surface-radius-angles $\SurfaceRadiusAngle_{\Upper}$ and $\SurfaceRadiusAngle_{\Lower}$) are calculated given the node coordinates using the cosine law as in \cref{eq:surface-radius-angles}.
...
The new coordinates after shifting along a vector $\Step\Shift$ which is directed under an angle of $\SurfaceVectorAngle$ with the upper surface line are calculated using cosine and sine law as in \cref{eq:new-node-coordinates}.
