...
Rings are discovered during a~\gls{BFGS} when a vertex is encountered that was already marked as visited.
To be able to construct geometric constraints on particle movement, a ring path must be defined.
A ring path is here considered to consist of two parts: a primary and a secondary path to the closing particle.
It is generally not unique, but this is also not necessary.
The ring path is constructed by backtracing the paths to the particle taken on the first and the current (second) visit.
The first common vertex is taken as the orign of the ring path.
This is not necessarily the smallest possible ring, which is also not required, although it is preferably for efficiency reasons.
However, this approach yields generally quite small rings due to the search order of the \gls{BFGS}.
