...
Additional requirements such as contact conditions are to be formulated as equality constraints and can be implicitly ensured by the solution procedure.
...
So, classic particle size averaging was found to be incorrect regarding neck size prediction, as the neck size influence of particle size is determined mainly by the smaller of the particles in contact, so a bias is introduced.
...
\begin{enumerate}
    ...
    \item In contacts between particles with high difference in surface energy, a drastic undercut near the neck occurs as discussed in \cref{subsec:parameter-study-asymmetric-surface-energy-ratio},
        which eventually leads to a crossing between grain boundary and surface, which is obviously not physical.
        This is a numerical effect due to the removal of volume from a node, where in fact no volume is left.
    \item The grain boundary can currently only consist of a single node (plus the two neck nodes) as was discussed in \cref{sec:contact-conditions},
...
\begin{enumerate}
    ...
    \item Transfer the approach to simulation of microstructure evolution of compact polycrystals regarding recrystallization, but also possibly with phase transformations.
\end{enumerate}
