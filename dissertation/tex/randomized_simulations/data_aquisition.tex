...
The fits are therefore repeated with the current shape function as discussed in \autoref{sec:statistical-powder-modeling}.
...
\autoref{fig:morphology/batches/hist_circular/all} shows the histogram of the fit results alongside the theoretical distribution fit.
Note the bimodal distribution shape of the histogram.

For particle size distributions it is common to use a Weibull distribution (\autoref{eq:weibull-particle-size}), with the shape parameter $k$, the scale parameter $S$ and the location parameter $L$.
The support of the distribution is $x \in [L, \infty)$.
As particle sizes must always be $> 0$, the location $L=0$ is used here.
\begin{equation}
    \Probability(x) = \frac{k}{S} (\frac{x - L}{S})^{k-1} \exp -(\frac{x - L}{T})^k
    \label{eq:weibull}
\end{equation}
The expectation $\Expectation$ of the Weibull distribution is given by \autoref{eq:weibull-expectation} with the gamma function $\GammaFunction$.
The expected radius $\Expectation{\Radius_0}$ will be used as the reference radius to calculate the characteristic time norm according to \autoref{eq:normalized_time}.
\begin{equation}
...
In the second case, the ovality term (\autoref{eq:f-ovality}) is added to the shape function.
...
\autoref{fig:morphology/batches/hist_oval/all} shows the histograms of the fit results alongside the theoretical distribution fits.
...
In the third case, first-order waves according to \autoref{eq:f-first-order-waves} are considered, too.
...
\autoref{fig:morphology/batches/hist_shape/all} shows the histograms of the fit results alongside the theoretical distribution fits.
...
Due to improved shape accuracy, the mean of particle size $\Radius_0$ is slightly decreasing from \qtyrange{767}{754}{\micro\meter}.
To ensure comparability, the reference radius for normalization will be fixed to the value of \qty{760}{\micro\meter} for the rest of this chapter.

\autoref{fig:comparison-particle-shape-fits} shows a typical particle from the dataset, which has been fitted by the three distinct approaches.
