...
In literature sintering behavior plots are often created with the normalized (dimensionless) time $\Normalized\Time$ as in \autoref{eq:parameter_study_normalized_time}, with the materials molar volume $\MolarVolume$ and the (equivalent) particle radius $\Radius_{\Particle}$.
The shown formulation refers to the surface properties of the material via it's surface diffusion coefficient $\DiffusionCoefficient_{\Surface}$ and surface energy $\InterfaceEnergy_{\Surface}$.
When grain boundary diffusion occurs, one may also refer to the grain boundary properties.
For asymmetrical contacts the properties of the second particle are also present.

\begin{equation}
    \Normalized\Time = \frac{\MolarVolume\DiffusionCoefficient_{\Surface}\InterfaceEnergy_{\Surface}}{\GasConstant\Temperature\Radius_{\Particle}^4} \cdot \Time
    \label{eq:parameter_study_normalized_time}
\end{equation}
