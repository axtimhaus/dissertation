...
\begin{figure}
    \begin{subfigure}{\linewidth}
        ...
        \label{fig:two_particle/studies/surface_energy_asymmetric/1.25000/evolution}
    \end{subfigure}
    \begin{subfigure}{\linewidth}
        \centering
...
\begin{figure}
    \begin{subfigure}{\linewidth}
        \centering
        \includegraphics[width=\linewidth]{sim/two_particle/studies/surface_energy_asymmetric/neck_size}
...
\begin{figure}
    \begin{subfigure}{\linewidth}
        \centering
        \includegraphics[width=\linewidth]{sim/two_particle/studies/surface_energy_asymmetric/shrinkage}
        \caption{Evolution Curves}
...
Due to the mathematical construction of the model, volume can be removed from a surface node in such an undercut despite there is too little volume left between the surface and the grain boundary, so after a critical time step, surface line and grain boundary cross.
...
As the surface diffusion is too slow to deliver enough volume to the neck, an undercut evolves.
This process is similar to the relation between surface energy and grain boundary in general determining the dihedral angle (see \cref{subsec:parameter-study-surface-boundary-energy}), with the main difference that here three different interface energies are in action. This leads to an asymmetric neck geometry.
Young's equation (\cref{eq:youngs-dihedral}) is only applicable for $\InterfaceEnergy_{\Surface2}/\InterfaceEnergy_{\Surface1} < \num{1.5}$ as for higher ratios equilibrium is not possible (see \cref{sec:diffusion}).
Remarkably for $\num{1} < \InterfaceEnergy_{\Surface2}/\InterfaceEnergy_{\Surface1} < \num{1.5}$ the equilibrium dihedral angle between the surfaces $\DihedralAngle_{\GrainBoundary}$ was present in later stages in the results, but the angles between surfaces and grain boundaries $\DihedralAngle_1$ and $\DihedralAngle_2$ did not fit to those proposed by \cref{eq:youngs-dihedral}.
So for example the simulation for $\InterfaceEnergy_{\Surface2}/\InterfaceEnergy_{\Surface1} = \num{1.25}$ (see \cref{fig:two_particle/studies/surface_energy_asymmetric/1.25000/evolution}) showed a $\DihedralAngle_{\GrainBoundary} \approx \ang{158}$, which is in coincidence with Young's equation proposal, but the simulated angles to the grain boundary were $\DihedralAngle_2 \approx \ang{96}$ (left) and $\DihedralAngle_1 \approx \ang{106}$ (right) compared to the proposed \ang{72} and \ang{130}.
