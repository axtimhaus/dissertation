...
\begin{figure}
    ...
    \caption{Step Counts and Simulation Time Consumption Used in Dependence on the Neck Remeshing Limit}
...
Agressive remeshing (higher limit values) is able to decrease the effort by factors from \numrange{2}{5}.

\begin{figure}
    \includegraphics{sim/two_particle/studies/neck_remeshing/shrinkage}
    \caption{Shrinkage Result Curves Obtained from the Simulations in Dependence on the Neck Remeshing Limit}
...
\begin{figure}
    ...
    \caption{Volume Loss During Simulation in Dependence on the Neck Remeshing Limit}
...
As has to be expected, the numerical volume loss shows significant dependence on the node count and remshing regime.
Coarse discretization and agressive remeshing lead to higher volume losses.
...
\begin{figure}
    ...
    \caption{Time Step Width During Simulation in Dependence on the Neck Remeshing Limit}
...
If the reagrd is only payed on late stages, low node counts lead to equivalent results.
The agressiveness of neck remeshing is of minor importance regarding the simulation accuracy, but is able to significantly decrease the computational effort.
Fot the sake of efficency, as to be expected, node counts as low as possible are preferred, as the effort increases exponentially with the node count.
