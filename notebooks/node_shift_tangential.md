...
<img src="../dissertation/img/model_development/node_shift_tangential.svg" style="background-color: white"/>
...
```python
al_ = sqrt(al**2 + ds**2 - 2 * al * ds * cos(pi - betal))
al_
```
...
```python
dV = au * ds * sin(betau) / 2 - al * ds * sin(pi - betal) / 2
dV
```
